...
/**
 * A TransactionSignature wraps an {@link io.xpydev.paycoinj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how Paycoin should work, thus, non-canonical signatures are now
     ...
     * of the type used by Paycoin we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToPaycoin() {
        try {
     ...
     */
    public static TransactionSignature decodeFromPaycoin(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // Paycoin encoding is DER signature + sighash byte.
        ...
        // In Paycoin, any value of the final byte is valid, but not necessarily canonical. See javadocs for

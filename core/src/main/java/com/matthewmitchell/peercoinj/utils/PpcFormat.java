...
import com.matthewmitchell.peercoinj.utils.PpcAutoFormat.Style;
import static com.matthewmitchell.peercoinj.utils.PpcAutoFormat.Style.*;
 ...
 *       {@link PpcFormat#format(Object)} method. 
 *   <li>Parse a value by passing a <code>String</code>-type
 *       representation of it to the {@link PpcFormat#parse(String)} method.</ol>
 *
 ...
 * be formatted, {@link PpcAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link PpcFixedFormat}.  
 *
 ...
 * <code>"Ꝑ"</code>.
 *
 * <p>The denomination used when formatting will be either peercoin, millicoin
 ...
 * locale, a value of one peercoin might be formatted as <pre>Ꝑ1.00</pre> where a value
 * exceeding that by one satoshi would be <pre>µꝐ1,000,000.01</pre>
 *
 ...
 *   <li>Use a {@link PpcFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class <code>PpcFormat</code>.  There
 ...
 * automatic-denominating <code>PpcAutoFormat</code> instance for your
 ...
 * the {@link PpcFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of micropeercoins:
 *
 * <blockquote><pre>PpcFormat m = PpcFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>PpcFormat milliFormat = PpcFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * <code>getInstance()</code> is one of the enumerated values of the
 * {@link PpcAutoFormat.Style} type, either <code>CODE</code> or
 * <code>SYMBOL</code>, then you will get a {@link PpcAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.
 *
 * <blockquote><pre>
 * PpcFormat s = PpcFormat.getInstance(SYMBOL);
 * Coin value = Coin.parseCoin("0.1234");
 * String mil = s.format(value);              <strong>// "₥Ꝑ123.40"</strong>
 * String mic = s.format(value.divide(1000)); <strong>// "µꝐ123.40"</strong>
 * </blockquote>
 *
 ...
 * PpcFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * PpcFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link PpcFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a <code>PpcFormat</code>
 * object.  Its {@link Builder#build()} method constructs and returns a <code>PpcFormat</code> instance
 ...
 *
 * <blockquote><pre>
 * PpcFormat.Builder builder = PpcFormat.builder();
 * builder.style(SYMBOL);
 * builder.symbol("B&#x5c;u20e6"); <strong>// unicode char "double vertical stroke overlay"</strong>
 * PpcFormat f = builder.build();
 * String out = f.format(COIN); <strong>// "Ꝑ1.00" depending on locale</strong>
 * </pre></blockquote>
 *
 ...
 * method.  This argument can be either a {@link com.matthewmitchell.peercoinj.core.Coin}-type object or a
 ...
 * number of peercoins.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link PpcFixedFormat} also accept a variable-length sequence of additional <code>int</code>
 ...
 *
 * <blockquote><pre>
 * PpcFormat f = PpcFormat.getCoinInstance();
 ...
 * {@link PpcFixedFormat#REPEATING_PLACES}, {@link
 * PpcFixedFormat#REPEATING_DOUBLETS} and {@link
 * PpcFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 * PpcFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * PpcFormat f = PpcFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * PpcFixedFormat} and {@link PpcAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link PpcAutoFormat} recognize
 ...
 * instances of {@link PpcFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * PpcFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link PpcFormat.Builder#pattern()} method.
 *
 * <p>The {@link PpcAutoFormat#parse(String)}</code> method of {@link PpcAutoFormat} (and of
 * {@link PpcAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * <code>PpcFormat</code> instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * PpcFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed <code>String</code>s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link PpcAutoFormat}
 * instance will infer a denomination of peercoins while a {@link PpcFixedFormat} will infer the
 ...
 * @see          com.matthewmitchell.peercoinj.core.Coin
 */
...
public abstract class PpcFormat extends Format {
    ...
    protected static final String COIN_SYMBOL_ALT = "Ꝑ";
    ...
    /* Scale is the number of decimal-places difference from same value in peercoins */
    /** A constant useful for specifying a denomination of peercoins, the <code>int</code> value
     *  <code>0</code>. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of millipeercoins, the <code>int</code>
     *  value <code>3</code>. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of micropeercoins, the <code>int</code>
     *  value <code>6</code>. */
    ...
    /**
     * <p>This class constructs new instances of {@link PpcFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * <code>Builder</code> object from the {@link PpcFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new <code>PpcFormat</code> object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking <code>build()</code> without invoking any
     * of the setting methods is equivalent to invoking {@link PpcFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override PpcFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            PpcFormat newInstance(Builder b) {
        ...
        private Style style = PpcAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new <code>PpcFormat</code> is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new <code>PpcFormat</code>
        ...
        public Builder style(PpcAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link PpcAutoFormat} instances.
         *
         ...
         *  denominated in peercoins.  For example, a denomination of millipeercoins is specified
         *  with a value of <code>3</code>.
         *
         * <p>The <code>PpcFormat</code> class provides appropriately named
         * <code>int</code>-type constants for the three common values, {@link PpcFormat#COIN_SCALE},
         * {@link PpcFormat#MILLICOIN_SCALE} {@link PpcFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(PpcAutoFormat.Style)} is invoked on a
         * <code>Builder</code>, then the <code>PpcFormat</code> will default to a
         * fixed-denomination of peercoins, equivalent to invoking this method with an argument
         * of <code>0</code>. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the <code>PpcFormat</code>
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link PpcFormat#symbols()}.
        ...
        /** Return a new {@link PpcFormat} instance.  The object returned will be configured according
         *  to the state of this <code>Builder</code> instance at the time this method is invoked. */
        public PpcFormat build() {
            PpcFormat f = variant.newInstance(this);
    ...
    protected PpcFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     */
    public static PpcFormat getInstance() { return getInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * symbol, for example, <code>"Ꝑ"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static PpcFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }
     ...
     * <code>"Ꝑ"</code>.  The returned object will format the fraction-part of numbers using
     ...
     * locale.
     */
    public static PpcFormat getSymbolInstance(int fractionPlaces) {
     ...
     * locale.
     */
    public static PpcFormat getCodeInstance(int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static PpcFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static PpcFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     * select denominational units based on each value being formatted, and will indicate those
     * units using a currency symbol, for example, <code>"µꝐ"</code>.
     */
    public static PpcFormat getSymbolInstance(Locale locale) {
     ...
     * <code>"µꝐ"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static PpcFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     */
    public static PpcFormat getInstance(Style style, int fractionPlaces) {
     ...
     */
    public static PpcFormat getInstance(Style style, Locale locale) {
     ...
     * fractional satoshis.
     */
    public static PpcFormat getInstance(Style style, Locale locale, int fractionPlaces) {
	return new PpcAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static PpcFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static PpcFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static PpcFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of peercoins, with the given minimum number of fractional
     ...
     */
    public static PpcFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static PpcFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static PpcFormat getMilliInstance(Locale locale) {
     ...
     */
    public static PpcFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static PpcFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static PpcFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static PpcFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static PpcFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static PpcFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static PpcFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static PpcFormat getInstance(int scale) {
     ...
     */
    public static PpcFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static PpcFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static PpcFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new PpcFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
     * Formats a peercoin monetary value and returns an {@link java.text.AttributedCharacterIterator}.
    ...
    /**
     * Formats a peercoin value as a number and possibly a units indicator and appends the
     ...
     * number of peercoins, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a peercoin value as a number and possibly a units indicator to a
     ...
     * <code>BigDecimal</code> is interpreted as that number of peercoins, rounded to the
    ...
    /**
     * Takes a peercoin monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a peercoin quantity of any type the
	...
	BigInteger satoshis;
        // the value might be peercoins or satoshis
    ...
    /**
      ...
      * {@link com.matthewmitchell.peercoinj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
     ...
     * String, then the number is interpreted as a quatity of peercoins.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * PpcAutoFormat} will interpret the parsed number as a quantity of peercoins, and instances
     * of {@link PpcAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * PpcFixedFormat#symbol()} or {@link PpcFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for peercoins configured for this object.  The
    ...
    /** Return the unprefixed international currency code for peercoins configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof PpcFormat)) return false;
        PpcFormat other = (PpcFormat)o;

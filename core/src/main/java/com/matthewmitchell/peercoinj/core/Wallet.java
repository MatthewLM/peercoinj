...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s PPC from our own wallet," +
                        " and sends us %s PPC", tx.getHashAsString(), Utils.peercoinValueToFriendlyString(valueSentFromMe),
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), new String(Hex.encode(tx.unsafePeercoinSerialize())));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), new String(Hex.encode(tx.unsafePeercoinSerialize())));
                }
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the Peercoin system when inflation drops. It also provides
         ...
         * 
         * <p>Peercoin: This is zero even though we add a required fee. This is an extra fee on top that we don't want.</p>
         */

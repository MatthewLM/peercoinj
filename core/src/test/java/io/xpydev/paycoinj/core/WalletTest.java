...
import io.xpydev.paycoinj.wallet.RiskAnalysis;
import io.xpydev.paycoinj.wallet.DeterministicKeyChain;
import io.xpydev.paycoinj.wallet.WalletFiles;
import io.xpydev.paycoinj.wallet.KeyChain;
import io.xpydev.paycoinj.wallet.DeterministicUpgradeRequiresPassword;
import io.xpydev.paycoinj.wallet.KeyTimeCoinSelector;
import io.xpydev.paycoinj.wallet.KeyChainGroup;
import io.xpydev.paycoinj.wallet.KeyBag;
import io.xpydev.paycoinj.wallet.WalletTransaction;
import io.xpydev.paycoinj.wallet.AllowUnconfirmedCoinSelector;
import io.xpydev.paycoinj.wallet.DefaultCoinSelector;
import io.xpydev.paycoinj.wallet.Protos;
import io.xpydev.paycoinj.testing.KeyChainTransactionSigner;
import io.xpydev.paycoinj.testing.TestWithWallet;
import io.xpydev.paycoinj.testing.NopTransactionSigner;
import io.xpydev.paycoinj.testing.MockTransactionBroadcaster;
import io.xpydev.paycoinj.testing.FakeTxBuilder;
import io.xpydev.paycoinj.crypto.KeyCrypterException;
import io.xpydev.paycoinj.crypto.KeyCrypterScrypt;
import io.xpydev.paycoinj.crypto.MnemonicException;
import io.xpydev.paycoinj.crypto.ChildNumber;
import io.xpydev.paycoinj.crypto.DeterministicKey;
import io.xpydev.paycoinj.crypto.KeyCrypter;
import io.xpydev.paycoinj.crypto.TransactionSignature;
import io.xpydev.paycoinj.core.Wallet.SendRequest;
import io.xpydev.paycoinj.params.MainNetParams;
import io.xpydev.paycoinj.script.Script;
import io.xpydev.paycoinj.script.ScriptBuilder;
import io.xpydev.paycoinj.script.ScriptOpCodes;
import io.xpydev.paycoinj.signers.StatelessTransactionSigner;
import io.xpydev.paycoinj.signers.TransactionSigner;
import io.xpydev.paycoinj.store.BlockStoreException;
import io.xpydev.paycoinj.store.MemoryBlockStore;
import io.xpydev.paycoinj.store.UnreadableWalletException;
import io.xpydev.paycoinj.store.WalletProtobufSerializer;
import io.xpydev.paycoinj.utils.ExchangeRate;
import io.xpydev.paycoinj.utils.Fiat;
import io.xpydev.paycoinj.utils.Threading;
import io.xpydev.paycoinj.wallet.WalletTransaction.Pool;
import com.google.common.collect.ImmutableList;
import com.google.common.collect.Lists;
import com.google.common.util.concurrent.ListenableFuture;
import com.google.protobuf.ByteString;
import io.xpydev.paycoinj.wallet.Protos.Wallet.EncryptionType;
import io.xpydev.paycoinj.core.AbstractBlockChain;
import io.xpydev.paycoinj.core.AbstractBlockChain;
import io.xpydev.paycoinj.core.AbstractWalletEventListener;
import io.xpydev.paycoinj.core.AbstractWalletEventListener;
import io.xpydev.paycoinj.core.Address;
import io.xpydev.paycoinj.core.Address;
import io.xpydev.paycoinj.core.Block;
import io.xpydev.paycoinj.core.Block;
import io.xpydev.paycoinj.core.BlockChain;
import io.xpydev.paycoinj.core.BlockChain;
import io.xpydev.paycoinj.core.Coin;
import io.xpydev.paycoinj.core.Coin;
import io.xpydev.paycoinj.core.ECKey;
import io.xpydev.paycoinj.core.ECKey;
import io.xpydev.paycoinj.core.InsufficientMoneyException;
import io.xpydev.paycoinj.core.InsufficientMoneyException;
...
import io.xpydev.paycoinj.core.StoredBlock;
import io.xpydev.paycoinj.core.Transaction;
import io.xpydev.paycoinj.core.Transaction;
import io.xpydev.paycoinj.core.TransactionConfidence;
import io.xpydev.paycoinj.core.TransactionConfidence;
import io.xpydev.paycoinj.core.TransactionInput;
import io.xpydev.paycoinj.core.TransactionInput;
import io.xpydev.paycoinj.core.TransactionOutPoint;
import io.xpydev.paycoinj.core.TransactionOutPoint;
import io.xpydev.paycoinj.core.TransactionOutput;
import io.xpydev.paycoinj.core.TransactionOutput;
import io.xpydev.paycoinj.core.Utils;
import io.xpydev.paycoinj.core.Utils;
import io.xpydev.paycoinj.core.VerificationException;
import io.xpydev.paycoinj.core.VerificationException;
import io.xpydev.paycoinj.core.Wallet;
import io.xpydev.paycoinj.core.Wallet;
...
import static io.xpydev.paycoinj.core.Coin.*;
import static io.xpydev.paycoinj.core.Utils.HEX;
import static io.xpydev.paycoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    // For peercoin we shall send two cents, one cent for fee and another to meet minimum output requirement
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, boolean testEncryption) throws Exception {
        ...
        // will attach a small fee. Because the Paycoin protocol makes it difficult to determine the fee of an
        ...
        try {
            ...
            assertEquals(valueOf(9, 1), e.missing); // ppcoin: Miss nine coins and cent fee
        }
    ...
    public void balance() throws Exception {
        ...
        spend.params = MainNetParams.get(); // ppcoin: Use normal selectable logic
        wallet.commitTx(spend);
        assertEquals(1, wallet.getPoolSize(WalletTransaction.Pool.PENDING));

        // Available and estimated balances should not be the same. We don't check the exact available balance here
        // because it depends on the coin selection algorithm.
        assertEquals(valueOf(4, 49), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // ppcoin: minus fee
    ...
    public void bounce() throws Exception {
        ...
        inbound2.addOutput(new TransactionOutput(params, inbound2, coinHalf.subtract(CENT), myAddress)); // ppcoin: Less a cent
        ...
        assertEquals(coin1.subtract(TWO_CENTS), wallet.getBalance()); // ppcoin: Two cents loss in two transactions
    }
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        // ppcoin: Make sure main net is used to make coins unselectable
        ...
        assertEquals(valueOf(1, 99), wallet.getBalance()); // ppcoin: minus fee
        assertTrue(wallet.isConsistent());
    }
    ...
    public void doubleSpends() throws Exception {
        ...
        assertEquals(value.subtract(CENT), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // ppcoin: minus cent fee
        // Now spend the change. This transaction should die permanently when the mutant appears in the chain.
        Transaction send3 = checkNotNull(wallet.createSend(address, value.subtract(TWO_CENTS))); // ppcoin: minus the first fee and then another required fee
    ...
    public void spendToSameWallet() throws Exception {
        ...
        // ppcoin: We will use MainNetParams so to make pending transaction unselectable
    ...
    public void pubkeyOnlyScripts() throws Exception {
        ...
        Transaction t2 = wallet.createSend(new ECKey().toAddress(params), value.subtract(CENT)); // ppcoin: minus fee
        assertNotNull(t2);
    ...
    public void autosaveImmediate() throws Exception {
        ...
        File f = File.createTempFile("peercoinj-unit-test", null);
    ...
    public void ageMattersDuringSelection() throws Exception {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            txns[i] = sendMoneyToWallet(COIN.add(CENT), AbstractBlockChain.NewBlockType.BEST_CHAIN); // ppcoin: Give room for fee
        }
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        // Spend it 
        SendRequest req = SendRequest.to(notMyAddr, CENT.multiply(11)); // ppcoin: minus cent fee
        ...
        // ppcoin: Check minimum fee is enforced
        ...
        // ppcoin: Check less than cent change is disposed of.
        ...
        SendRequest request6 = SendRequest.to(notMyAddr, Coin.COIN.subtract(TWO_CENTS)); // ppcoin: Extra cent for fee
        ...
        // We optimize for priority, so the output selected should be the largest one
        assertEquals(Coin.COIN.subtract(CENT), spend6.getOutput(0).getValue().add(spend6.getOutput(1).getValue())); // ppcoin: minus cent fee

        // ppcoin: All balance minus fee gives no change
        ...
        // ppcoin: Used 1.00 coin, now 0.9 due to fee
        ...
        // ppcoin: Check less than 1kb transaction
        ...
        assertEquals(outValue19, COIN.add(CENT)); // ppcoin: We used two cents remember?
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        SendRequest request1 = SendRequest.to(notMyAddr, balance.subtract(CENT).subtract(SATOSHI)); // ppcoin: minus fee
        wallet.completeTx(request1);
        assertEquals(SATOSHI.add(CENT), request1.tx.getFee()); // ppcoin: cent fee added
    ...
    public void feePerKbCategoryJumpTest() throws Exception {
        ...
        // ppcoin: With inputs of Coin plus cent we can pay 1.00 with no change
        ...
        // ppcoin: We can pay 1.001 with coin and two cents but change is discarded
        ...
        // ppcoin: We can pay 0.991 with coin and one cent but change is discarded so two cents are used instead to allow for change
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        assertEquals(COIN.subtract(CENT.multiply(2)), request1.tx.getOutput(1).getValue()); // ppcoin: cent fee
    ...
    public void testEmptyWallet() throws Exception {
        ...
        Transaction tx = createFakeTx(params, TWO_CENTS, myAddress); // ppcoin: Need an extra cent for fee
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
    ...
    public void keyRotationRandom() throws Exception {
        ...
        assertEquals(CENT, tx.getOutput(0).getValue()); // ppcoin: Two cents minus fee to one cent remaining.
    ...
    public void keyRotationHD2() throws Exception {
        ...
        assertEquals("PKRGKjWepp8UcdgJeTgohaxD42dPje7Av3", usedKey.toAddress(params).toString());
    ...
    public void sendRequestMemo() throws Exception {
        receiveATransaction(wallet, myAddress);
        SendRequest sendRequest = SendRequest.to(myAddress, Coin.COIN.subtract(CENT)); // ppcoin: minus fee

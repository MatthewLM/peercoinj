...
public class PaymentProtocolTool {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Provide a peercoin URI or URL as the argument.");
            return;
        }
        dump(args[0]);
    }
    ...
    private static void dump(String arg) {
        try {
            ...
            } else if (uri.getScheme().equals("peercoin")) {
                PeercoinURI bcuri = new PeercoinURI(arg);
                final String paymentRequestUrl = bcuri.getPaymentRequestUrl();
                if (paymentRequestUrl == null) {
                    System.err.println("No r= param in peercoin URI");
                    return;
                }
                session = PaymentSession.createFromPeercoinUri(bcuri).get();
            } else {
            ...
            StringBuilder output = new StringBuilder(
                    format("Peercoin payment request, version %d%nDate: %s%n", version, session.getDate()));
